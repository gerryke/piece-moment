(function () {
  ...
  var I18N = {
    ...
    "pv.lead":       { en:"Pieceful Moment is designed to be calm — and that calm extends to how it treats your privacy: we don't collect your personal data, we don't show ads, and the app works fully offline.", zh:"片刻被设计得很安静——这份安静也包括它对待你隐私的方式：我们不收集你的个人数据，不展示广告，App 完全可以离线使用。", zht:"片刻拼圖被設計得很安靜——這份安靜也包括它對待你隱私的方式：我們不收集你的個人資料，不顯示廣告，App 完全可以離線使用。", ja:"Pieceful Moment は静けさを大切にしています——その静けさは、プライバシーの扱いにも表れています。個人データを収集せず、広告も表示せず、アプリは完全にオフラインで動作します。" },
    "pv.s1.h":       { en:"Information we collect", zh:"我们收集的信息", zht:"我們收集的資訊", ja:"収集する情報" },
    "pv.s1.p":       { en:"None. Pieceful Moment does not require an account, does not ask for your name or email, and does not collect analytics or usage tracking. We have no servers that store your information.", zh:"没有。片刻不需要注册账号，不索取你的姓名或邮箱，也不收集分析或使用行为追踪。我们没有任何存储你信息的服务器。", zht:"沒有。片刻拼圖不需要註冊帳號，不索取你的姓名或電子郵件，也不收集分析或使用行為追蹤。我們沒有任何儲存你資訊的伺服器。", ja:"ありません。Pieceful Moment はアカウント登録を必要とせず、氏名やメールアドレスを尋ねることもなく、分析や利用状況のトラッキングも行いません。あなたの情報を保存するサーバーもありません。" },
    ...
    "pv.s4.p":       { en:"Pieceful Moment is family-friendly: no ads, no purchases that target children, and no data collection — making it safe for players of all ages.", zh:"片刻适合全家使用：没有广告、没有针对儿童的购买项目、也不收集数据——适合各个年龄段的玩家。", zht:"片刻拼圖適合全家使用：沒有廣告、沒有針對兒童的購買項目，也不收集資料——適合各個年齡層的玩家。", ja:"Pieceful Moment は家族みんなで楽しめます。広告や子ども向けの課金はなく、データ収集も行わないため、あらゆる年齢の方が安心して遊べます。" },
  ...
  function apply(lang) {
    ...
    document.querySelectorAll("[data-localized-shot]").forEach(function (img) {
      img.src = "assets/img/shots/v8/" + lang + "/" + img.dataset.localizedShot + ".jpg?v=2";
    });
